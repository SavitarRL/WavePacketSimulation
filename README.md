...
$`J`$ is the interaction strength, $`m,n`$ the lattice indices, $`\hat^{c}^\dagger(c^\dagger)`$ are fermionic creation(annihilation) operators, $`\sigma_x,\sigma_y`$ are Pauli operators and $`\theta,\phi`$ are phase factors.
